...
locals {
  ...
  jenkins_proxy_http = element(
    split(
      ":",
      replace(
        replace(data.aws_ssm_parameter.proxy_http.value, "http://", ""),
        "https://",
        "",
      ),
    ),
  ...
  iam_policy_names_sufix  = local.auto_IAM_mode == 1 ? local.auto_iam_policy_names_sufix : ""
